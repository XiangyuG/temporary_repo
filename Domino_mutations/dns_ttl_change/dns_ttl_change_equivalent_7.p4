...
blackbox stateful_alu test_stateful_alu_1_1_blackbox {
    ...
    condition_hi              : ((0-ipv4.p_seen00)-0)<0;
    update_lo_1_predicate     : false;
    update_lo_1_value         : (alu_lo)-(14);
...
// Stateful ALU blackbox
blackbox stateful_alu test_stateful_alu_2_4_blackbox {
    
    
    reg                       : reg_4;
    condition_lo              : ((0-ipv4.p__br_tmp6)-alu_hi)<0;
    condition_hi              : ((ipv4.p__br_tmp6-alu_lo)+2) !=0;
    update_lo_1_predicate     : (condition_lo);
    update_lo_1_value         : (ipv4.p__br_tmp7);
    update_lo_2_predicate     :  not (condition_lo);
    update_lo_2_value         : (ipv4.p__br_tmp7)+(alu_lo);
    update_hi_1_predicate     : ( not (condition_hi)) and (condition_lo);
    update_hi_1_value         : (2);
    update_hi_2_predicate     : false;
    update_hi_2_value         : (19)-(29);
    output_predicate          : 1;
    output_value              : 0;
    output_dst                : 0;
...
// Stateful ALU Action
action test_stateful_alu_2_4_action () {
    test_stateful_alu_2_4_blackbox.execute_stateful_alu(0);
    // TODO: Replace 0 with appropriate value for array-based registers. The
    // appropriate value can be determined by parsing the .c file using the
    // Domino compiler.
}
...
// Stateful ALU table
@pragma ignore_table_dependency test_stateful_alu_2_4_table
@pragma stage 2
table test_stateful_alu_2_4_table {
    actions {
        test_stateful_alu_2_4_action;
    }
    default_action: test_stateful_alu_2_4_action;
}
...
control ingress {
    // Call all the required ALUs.
          ...
          apply(test_stateful_alu_2_4_table);
